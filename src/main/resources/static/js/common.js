...
$(function() {
	...
	$(document.body).on("click", ".btn_a_change_day_submit", function(e) {
				...
				if(!reg.test(inputs.eq(0).val())){
					isChecked=false;
					layersMoretips('密码重置天数为正整数',inputs.eq(0));
				}
				if(isChecked==false)return false;
				showloading();
...
function richtext(editor) {
    editor.customConfig.showLinkImg = false //手动关闭粘贴样式
    // 隐藏“网络图片”tab
    editor.customConfig.uploadImgMaxSize = 3 * 1024 * 1024;
	// 限制一次最多上传 5 张图片
	editor.customConfig.uploadImgMaxLength = 30;
	editor.customConfig.pasteFilterStyle = false
	editor.customConfig.uploadImgTimeout = 30000 //图片上传超时时间
	editor.customConfig.uploadImgServer = loginuserdata.body.file_server + 'uploades'
	editor.customConfig.uploadFileName = 'files'//自定义上传参数
...
function initFileInput(ctrlName, check, data) {
	...
	control.fileinput({
		language: 'zh', //设置语言
		uploadUrl: loginuserdata.body.file_server  + "upload?format=json", //上传的地址
		...
		initialPreviewAsData: true, //是否显示缩略图
		showClose: false, //是否隐藏关闭按钮
		dropZoneEnabled: true, //是否显示拖拽区域
		showRemove: false, //是否显示删除按钮
		showBrowse: check, //是否显示选择按钮
		layoutTemplates: layoutTemplates, //隐藏预览图删除图标  隐藏拖拽图标
	...
	}).on("filesuccessremove", function(event, id) { //自动上传
		getFileInput.dele(ctrlName, id)
	}).on("filepreremove", function(event, id) { //自动上传
		getFileInput.dele(ctrlName, id)
	})
...
function DrawPageBase(){
	...
	$(document.body).on("click", ".btn_a_change_pwd_submit", function(e) {
				...
				var inputs=$("#div_change_pwd input");
				if(!FormDataCheck.isUserName(inputs.eq(0).val())){
					layersMoretips('以字母开头至少6位',inputs.eq(0));
					isChecked=false;
					return false;
				}
				if(inputs.eq(0).val()==''||inputs.eq(0).val().length<3){layersMoretips('必填项，最少3位',inputs.eq(0));isChecked=false;return false;}
...
function drawfiles(files,isdelete){
	...
	$.each(files, function(i, item) {
		...
		strtext += ("<div class='fileer'>" + item.user_name + "</div>");
		strtext += ("<div class='filename'><a href='"+(loginuserdata.body.file_server + item.file_id)+"' target='_blank' data-id='" + item.file_id + "' title='点击下载：" + item.file_id + "'>" + formartFileName(item.file_name) + "</a>  (" + formartFileSize(item.file_size) + "kb)</div>");
		strtext += ("<div class='clearfx'></div>");
